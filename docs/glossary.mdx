...
**A Witness is a module that onboards data.**

A Witness collects data from an outside source, either virtual or physical.  Witnesses are the part of XYO that fall under the idea of an Oracle.

For example, a "Temperature" witness would query a temperature sensor and generate a specific type of payload that contains the data returned by the sensor.
